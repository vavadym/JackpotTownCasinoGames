...
class FortuneViewController: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        let imageSliceContent = Slice.ContentType.assetImage(name: "Group", preferences: imagePreferences)

        let slice = Slice(contents: [imageSliceContent])
        let slice1 = Slice(contents: [imageSliceContent])
        let slice2 = Slice(contents: [imageSliceContent])
        let slice3 = Slice(contents: [imageSliceContent])
        let slice4 = Slice(contents: [imageSliceContent])
        let slice5 = Slice(contents: [imageSliceContent])
